...
vi.mock('@/components/chat/chat-area', async () => {
  ...
  return {
    ChatArea: React.forwardRef(function MockChatArea(props: Record<string, unknown>, ref) {
      // eslint-disable-next-line react-hooks/immutability -- test mock records the props it renders with
...
describe('PlaybackChromeRoot element-reference ownership', () => {
  ...
  it('selects the stage snapshot whiteboard and sends an identity-only reference', async () => {
    ...
    expect(container.querySelector('[data-testid="owner-pill"]')?.textContent).toMatch(
      /whiteboard\.title · Text ·\s*First grounded fact/u,
    );
    expect((mocks.chatAreaProps?.canComposerSubmit as () => boolean)()).toBe(true);
    click('send');
  ...
  it('keeps a stale whiteboard selection visible and blocks sending until reselected or removed', async () => {
    ...
    await rerenderOwner();
    expect((mocks.chatAreaProps?.canComposerSubmit as () => boolean)()).toBe(false);
    expect(mocks.sendMessage).not.toHaveBeenCalled();
    expect(container.querySelector('[data-testid="owner-pill"]')).not.toBeNull();
    act(() =>
      (
        container.querySelector(
          '[data-testid="owner-pill"] [aria-label="chat.elementReference.clear"]',
        ) as HTMLElement
      ).click(),
    );
    expect((mocks.chatAreaProps?.canComposerSubmit as () => boolean)()).toBe(true);
