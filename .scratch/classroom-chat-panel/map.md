...
一份**经原型确认的实现级设计规格**：把学生发言入口从左下角横条搬进右侧"对话"tab 下方，做成标准 AI 聊天式的输入区；横条上不再保留任何学生控件（麦克风、气泡按钮、头像全部撤走——这里说的是**学生自己的**控件；学生 agent 的头像条保留，见"已拍板的形状"）；全屏讲课时输入条移进**右侧黑框**（幻灯片区之外，任何屏幕比例下都不遮挡课件——全屏会强制收起右侧面板，`PlaybackChromeRoot.tsx:613-614`）；历史/新对话按"对话列表"心智重做，课堂自动会话并入同一列表并带类型标记。规格细到"改哪个文件、改成什么结构、哪些测试跟着改"。
...
- **已拍板的形状**（grilling 三轮的结论，细节见各票）：
  ...
  - 历史入口是**面板顶部的浮层**（对话常驻，历史从顶部盖下来一层；原型已确认，变体 B）；列表按"今天/昨天/更早"倒序分组；条目 = 标题（学生首句，可重命名）+ 类型与状态小图标（逐条时间戳去掉，时间只由分组表达，精确时间放悬停 title）；有"新对话"按钮。
  - （2026-09-22 修订 P1）**不做隐藏，也不做删除**：条目菜单只留"改名"。原决定"删除做成本地隐藏"的依据是"后端不能真删"，但再权衡后不值——隐藏要额外引一个 device KV 键、一个"已隐藏（N）"入口和三条边界（未读 / 复活 / 进行中），换来的是"本地视觉整理"；而课堂里列表只增不减本身说得通（提问记录是学习轨迹）。将来真想要再加，不影响现在的结构。
  ...
  - （G1 修订，2026-09-22 定案）横条右栏**只撤下半截**：撤掉学生自己的控件（麦克风 / 气泡按钮 / 用户头像 / 冷却三点波 / 那处"轮到你了"提示），**保留上部的学生 agent 头像条**。理由：老师那条邀请卡锚在左栏（`teacherAvatarRef`，`roundtable/index.tsx:1176`）不受影响，但学生 agent 那条的唯一锚点就是这条头像条（`studentAvatarRefs` 注册点 `:1919-1922` → 卡片 `:2046`），整栏撤走会让非全屏的学生邀请卡**直接不渲染**（`ProactiveCard` `proactive-card.tsx:63-80`：无锚点即 return）；而非全屏今天没有合格的替代锚点（全屏 dock 是单槽且仅全屏存在），这条头像条同时还是全量学生名册、人设悬停卡、说话 / 思考指示的家。P1 的"两栏布局"据此改成"左栏老师 + 中栏 + 右侧窄头像条"，横条高度 192px 不变。
...
<!-- 原型已把下面六项定型，它们随 S1 写进规格，不再单独立票：
     ...
     空对话屏（图标 + "开始一段新对话"）
     列表条目（标题 + 类型徽章 + 状态点，悬停出"改名"；逐条时间戳按 G1 去掉）
     窄面板折行（按钮收成图标，标签隐藏）
     横条右栏（只撤下半截的学生控件，保留上部学生 agent 头像条；横条仍是 192px） -->

## Out of scope

- 后端与存储层改动：单条会话真删除、跨设备历史（需账号系统）——用户明确"后端不变"。**隐藏也不再作为替代方案**（2026-09-22：本次干脆不做隐藏，列表只增不减）
- 建议问题卡——用户明确不要
- 模型选择与联网搜索开关——用户明确不要
- 笔记 tab 的任何改动
