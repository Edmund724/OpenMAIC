...
一份**经原型确认的实现级设计规格**：把学生发言入口从左下角横条搬进右侧"对话"tab 下方，做成标准 AI 聊天式的输入区；横条上不再保留任何学生控件（麦克风、气泡按钮、头像全部撤走）；全屏讲课时例外——舞台上保留一套输入条（全屏会强制收起右侧面板，`PlaybackChromeRoot.tsx:613-614`）；历史/新对话按"对话列表"心智重做，课堂自动会话并入同一列表并带类型标记。规格细到"改哪个文件、改成什么结构、哪些测试跟着改"。

原型先行（用户已确认）：先做出可点的原型确认观感，再固化成规格。
...
- **已拍板的形状**（grilling 三轮的结论，细节见各票）：
  ...
  - 历史入口是**面板内两屏切换**（列表屏 ↔ 对话屏）；列表按"今天/昨天/更早"倒序分组；条目 = 标题（学生首句，可重命名）+ 时间 + 类型与状态小图标；有"新对话"按钮；删除做成**本地隐藏**。
  - 点开已结束的旧对话**可以接着问**；讲课记录（lecture 会话）**不进列表**。
  - 学生随时可问，但"轮到你了"时输入区高亮 + 对话 tab 琥珀点。
  - 暂停讲课只在学生**真正开始输入**时触发（聚焦/按键/开始录音），常驻本身不暂停。
  - 全屏那套输入条与面板输入框是**同一个组件两处渲染**，草稿互不共享。
...
- 输入区的最终视觉规格（一行还是两行、引用课件入口摆哪、录音态长什么样）——待 P1 原型
- "轮到你了"在面板输入区里的高亮形式——待 P1 原型
- 空对话屏长什么样（现在是"暂无对话"两行字）——待 P1 原型
- 列表条目上类型/状态图标的具体选择与文案——待 P1 原型
- 列表条目要不要保留逐条时间戳（R1 发现六个产品都去掉了、只靠分组表达，而已定形状里带了时间）——待 P1 原型
- 窄面板（240px）下输入区折成两层时的具体排布——待 P1 原型
- 横条右栏空出后的收尾：高度动不动、气泡舞台怎么占位——待 P1 原型
- 两屏切换与引擎自动开新会话的手感细节（切回对话屏的时机与动画）——待 G1 与 P1
