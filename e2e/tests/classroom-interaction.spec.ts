...
test.describe('Classroom Interaction', () => {
  ...
  test('caps the composer draft height and keeps the draft across a blur', async ({ page }) => {
    const classroom = new ClassroomPage(page);
    await classroom.goto(TEST_STAGE_ID);
    await classroom.waitForLoaded();

    // T expands the panel, opens the chat tab, and focuses the composer.
    ...
    const readMetrics = () =>
      textarea.evaluate((element) => {
        ...
        if (!element.closest('[data-testid="chat-composer"]')) {
          throw new Error('Could not find the composer box');
        }
    ...
    await test.info().attach('composer textarea metrics', {
      body: JSON.stringify(longDraftMetrics, null, 2),
      contentType: 'application/json',
    });

    const metricSummary = JSON.stringify(longDraftMetrics);
    expect(longDraftMetrics.inlineHeight).toBe('100px');
    expect(longDraftMetrics.computedMaxHeight).toBe('100px');
    expect(
      longDraftMetrics.computedOverflowY,
      `Composer textarea metrics: ${metricSummary}`,
    ).toBe('auto');
    ...
    await textarea.fill(longDraft);
    // Escape drops focus; the draft belongs to the panel, so T brings it back whole.
